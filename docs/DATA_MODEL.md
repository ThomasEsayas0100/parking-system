...
| `AuditAction` | `CHECKIN`, `CHECKOUT`, `EXTEND`, `OVERSTAY_START`, `OVERSTAY_PAYMENT`, `GATE_OPEN`, `SPOT_FREED`, `REMINDER_SENT`, `OVERSTAY_ALERT`, `SUSPICIOUS_ENTRY`, `GATE_DENIED`, `ALLOWLIST_ENTRY`, `LAYOUT_SAVED`, `LAYOUT_RESTORED`, `PAYMENT_EXPIRED` |
...
5. **Payment status lifecycle**: `COMPLETED` → `VOIDED` (admin cancel) or synced from QB via `POST /api/admin/qb-reconcile`. QB is the authoritative source of truth — the reconcile endpoint detects voids, partial refunds, and full refunds in QB and writes them back to `Payment.status` / `refundedAmount` / `refundExternalId`. `refundedAmount ≤ amount` is a model invariant but not DB-enforced; the reconcile path enforces it in code. The incoming-partial guard (`verifyAndClaimInvoice` rejects `status.partial`) is separate — it blocks check-in when a QB invoice is only partially paid, and is always active regardless of reconciliation.
